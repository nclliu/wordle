...
(define (score guess answer)
  (local [(define STATUS-LIST
            (mymap2 (explode guess) (explode answer)
                    (λ (a b) (if (string=? a b) LS-RIGHT
                                 (if (string-in-list? a (explode answer))
                                     LS-MISPLACED
                                     LS-WRONG)))))]
    (mymap2 (explode guess) STATUS-LIST make-pair)))
...
(define (partial-guess->full-guess guess len)
  (if (= len (string-length guess)) guess
      (implode (append (explode guess)
                       (build-list (- len (string-length guess)) (λ (n) " "))))))
...
(define (key-wordle wordle ke los len)
  (cond [(string=? "\b" ke) (ke-back wordle ke)]
        [(string=? "\r" ke) (ke-ret wordle ke los)]
        [else (ke-rest wordle ke len)]))

; ke-back : WS KeyEvent -> WS
; handles backspace to produce new world state
(define (ke-back wordle ke)
  (if (> (string-length (wordle-current wordle)) 0)
      (make-wordle (wordle-past wordle)
                   (substring (wordle-current wordle)
                              0 (sub1 (string-length (wordle-current wordle)))))
      wordle))

(check-expect
 (ke-back WS-START LITTLE-WORDS)
 WS-START)

(check-expect
 (ke-back WS-TYPE1 LITTLE-WORDS)
 WS-START)

(check-expect
 (ke-back WS-TYPE2 LITTLE-WORDS)
 WS-TYPE1)

(check-expect
 (ke-back WS-TYPE3 LITTLE-WORDS)
 WS-TYPE2)

(check-expect
 (ke-back WS-TYPE-BAD LITTLE-WORDS)
 WS-TYPE2)

(check-expect
 (ke-back WS-WORD1 LITTLE-WORDS)
 WS-WORD1)

; ke-ret : WS KeyEvent [List-of String] -> WS
; handles return to produce new world state
(define (ke-ret wordle ke los)
  (if (string-in-list? (wordle-current wordle) los)
      (make-wordle (append (wordle-past wordle) (list (wordle-current wordle))) "")
      wordle))
(check-expect
 (ke-ret WS-START "\r" LITTLE-WORDS)
 WS-START)

(check-expect
 (ke-ret WS-TYPE1 "\r" LITTLE-WORDS)
 WS-TYPE1)

(check-expect
 (ke-ret WS-TYPE2 "\r" LITTLE-WORDS)
 WS-TYPE2)

(check-expect
 (ke-ret WS-TYPE3 "\r" LITTLE-WORDS)
 WS-WORD1)

(check-expect
 (ke-ret WS-TYPE-BAD "\r" LITTLE-WORDS)
 WS-TYPE-BAD)

(check-expect
 (ke-ret WS-WORD1 "\r" LITTLE-WORDS)
 WS-WORD1)

; ke-rest : WS KeyEvent Nat -> WS
; handles other keys to produce new world state
(define (ke-rest wordle ke len)
  (local [(define (guess-done? curr)
            (= (string-length curr) len))]
    (if (guess-done? (wordle-current wordle)) wordle
        (if (string-alphabetic? ke)
            (make-wordle (wordle-past wordle)
                         (string-append (wordle-current wordle) (string-upcase ke)))
            wordle))))
                   
(check-expect
 (ke-rest WS-START "C" 3)
 WS-TYPE1)
...
(check-expect
 (ke-rest WS-START "1" 3)
 WS-START)

(check-expect
 (ke-rest WS-TYPE1 "A" 3)
 WS-TYPE2)

(check-expect
 (ke-rest WS-TYPE1 "A" 1)
 WS-TYPE1)

(check-expect
 (ke-rest WS-TYPE1 "2" 3)
 WS-TYPE1)

(check-expect
 (ke-rest WS-TYPE2 "T" 3)
 WS-TYPE3)

(check-expect
 (ke-rest WS-TYPE2 "X" 3)
 WS-TYPE-BAD)

(check-expect
 (ke-rest WS-TYPE3 "S" 3)
 WS-TYPE3)          
...
; guessed
(define (end-wordle? wordle answer chances)
  (or (string-in-list? answer (wordle-past wordle))
      (= chances (length (wordle-past wordle)))))
...
(check-expect (draw-end-wordle WS-WORD3 "BAD" 4)
              (overlay
               (text (string-append "You've won using 3 guesses!") 20 "black")
               (empty-scene 300 300)))

(check-expect (draw-end-wordle WS-WORD3 "HAY" 3)
              (overlay
...
(define (draw-end-wordle wordle answer chances)
  (cond [(string-in-list? answer (wordle-past wordle))
         (overlay
          (text (string-append "You've won using "
                               (number->string (length (wordle-past wordle))) " guesses!") 20 "black")
          (empty-scene 300 300))]
        [else (overlay
               (text "You lost ;-;" 20 "black")
               (empty-scene 300 300))]))
...
(check-expect (draw-wordle WS-WORD3 "ETA" 4)
              (stack/v (append (map row->image
                                    (map (λ (n)
                                           (map (λ (n) (make-pair n lsp->image))
                                                (score n "ETA"))) (wordle-past WS-WORD3)))
                               (list (partial-guess->image "" (string-length "ETA"))))))
(check-expect (draw-wordle WS-TYPE3 "BAD" 3)
              (stack/v (append (list (partial-guess->image
                                      (wordle-current WS-TYPE3) 3))
                               (build-list 2 (λ (n) (partial-guess->image "" 3))))))
...
(define (draw-wordle wordle answer chances)
  (local [(define BLANK-ROWS (- chances 1 (length (wordle-past wordle))))
          (define guesses
            (append (map row->image
                         (map (λ (n)
                                (map (λ (n) (make-pair n lsp->image))
                                     (score n answer))) (wordle-past wordle))) ; previous guesses 
                    (list (partial-guess->image
                           (wordle-current wordle) (string-length answer))) ; current guess 
                    (build-list BLANK-ROWS
                                (λ (n) (partial-guess->image "" (string-length answer))))))]      
    (stack/v guesses)))
